...
import { customerOrderSchema } from './schema';
...
const requiredOrderFieldsSchema = z.unknown().superRefine((data, ctx) => {
  ...
  const requiredFields = ['orderNumber', 'orderDate', 'customer', 'items'] as const;
  ...
  data.items.forEach((item, index) => {
    ...
    const hasQuantity =
      ('qty' in item && item.qty !== undefined && item.qty !== null) ||
    ...
    const hasWeight =
      ('weight_oz' in item && item.weight_oz !== undefined && item.weight_oz !== null) ||
    ...
    const hasDimensions =
      ('dims' in item && item.dims !== undefined && item.dims !== null) ||
      ('dimensions' in item &&
 ...
 */
export function validateRequiredOrderFields(data: unknown): void {
  const result = requiredOrderFieldsSchema.safeParse(data);
  if (!result.success) {
    throw new ValidationError(formatIssue(result.error.issues[0]));
  }
}
 ...
 */
export function parseValidatedCustomerOrder(data: unknown): CustomerOrder {
  const result = customerOrderSchema.safeParse(data);
  if (!result.success) {
    throw new ValidationError(formatIssue(result.error.issues[0]));
  }
  return result.data;
...
export function validateOrder(order: CustomerOrder): void {
  validateRequiredOrderFields(order as unknown);
  parseValidatedCustomerOrder(normalizeCustomerOrder(order));
}
